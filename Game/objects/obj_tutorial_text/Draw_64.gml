...
// --- FUNÇÃO LOCAL DE DESENHO (ajustada para centralizar correto) ---
...
if (atual_index == 1) {
    ...
    var w_key = sprite_get_width(spr_shift) * esc_s;

    var total = w1 + gap + w_key + gap + w2;
    var xx = pos_x - (total * 0.5);

    draw_text_shadow(xx, pos_y, t1, esc_t, _a, off); xx += w1 + gap;
    draw_key(spr_shift, 0, xx, pos_y, 0, _a);       xx += w_key + gap;
    draw_text_shadow(xx, pos_y, t2, esc_t, _a, off);
}

//if (atual_index == 2) {
 //   var t1 = "Aperte", t2 = "para pular";
 //   var w1 = string_width(t1) * esc_t, w2 = string_width(t2) * esc_t;
//var w_key = sprite_get_width(spr_arrows) * esc_s;
  ...
  //  var xx = pos_x - (total * 0.5);
   ...
   // draw_text_shadow(xx, pos_y, t1, esc_t, _a, off); xx += w1 + gap;
   // draw_key(spr_arrows, 0, xx, pos_y, 0, _a);      xx += w_key + gap;
    //draw_text_shadow(xx, pos_y, t2, esc_t, _a, off);
//}
